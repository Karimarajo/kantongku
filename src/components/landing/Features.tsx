...
// landing-page-revisi Task 3 — rebuilt as a card-based "curated product
// snippet" dashboard style: 7 of the 8 cards carry a small floating/tilted
// frame meant to hold a CROPPED excerpt of a real app screenshot (not a
// full raw screenshot pasted in) — the 8th ("Beli Sekali, Pakai
// Selamanya") is icon + copy only, per the prompt.
//
// Asset files live in src/assets/screenshots/ (folder created, currently
// empty — see its .gitkeep) — none exist yet, the owner provides them
// separately. A real `import` of a file that doesn't exist yet would break
// the Vite build, so every import below is commented out and every
// `screenshot` field left `undefined` for now; FeatureScreenshotFrame
// renders a placeholder in that case. To wire up a real asset: drop the
// file in src/assets/screenshots/, uncomment its import line, and set the
...
function FeatureScreenshotFrame({ src, alt, rotate }: { src?: string; alt: string; rotate: string }) {
  return (
    <div
      className={`w-full h-28 rounded-xl border border-overlay/10 shadow-lg overflow-hidden bg-surface-variant/40 ${rotate} transition-transform duration-300 hover:rotate-0`}
    >
      {src ? (
...
export default function Features() {
  return (
    <section className="w-full px-6 py-16 bg-body-bg">
      <div className="max-w-5xl mx-auto flex flex-col gap-10">
        <div className="flex flex-col items-center gap-2 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-on-surface">Fitur Unggulan</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {FEATURES.map((f, i) => {
            const Icon = f.icon;
            return (
              <div key={i} className="flex flex-col gap-3 bg-surface border border-overlay/10 rounded-2xl p-6 overflow-hidden">
